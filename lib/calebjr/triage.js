...
export async function friendlyComplete(summary) {
  try {
    const res = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `A change to our website was just finished. In 1-2 warm, friendly sentences with absolutely NO technical jargon (don't say code, PR, deploy, branch, build, repo, commit), tell the person who asked that it's done and plainly describe what changed, like you're explaining to a non-technical beginner. Do not start with their name (it's added separately). The change: "${summary}"`,
      }],
    })
