...
// A failed task: pick a repo + Retry (relaunch) / Dismiss.
export function failedBlocks(a, repos = []) {
  const ctx = [a.cursorUrl ? `<${a.cursorUrl}|details>` : null].filter(Boolean).join(' · ')
  const elements = []
  if (repos.length) {
    const repoSelect = {
      type: 'static_select',
    ...
    const selected = repos.find((r) => r.key === a.repoName)
    if (selected) repoSelect.initial_option = { text: { type: 'plain_text', text: selected.key }, value: selected.key }
    elements.push(repoSelect)
  }
  elements.push(
    { type: 'button', action_id: 'retry', text: { type: 'plain_text', text: 'Retry' }, style: 'primary', value: String(a._id) },
    { type: 'button', action_id: 'skip', text: { type: 'plain_text', text: 'Dismiss' }, style: 'danger', value: String(a._id) }
  )
  return [
    { type: 'section', text: { type: 'mrkdwn', text: `:warning: *Task failed* — ${a.summary}` } },
    ...(ctx ? [{ type: 'context', elements: [{ type: 'mrkdwn', text: ctx }] }] : []),
    { type: 'actions', block_id: 'failed', elements },
  ]
}
