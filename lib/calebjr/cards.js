...
// A failed task: Retry (relaunch) / Dismiss.
export function failedBlocks(a) {
  const ctx = [a.repoName ? `→ ${a.repoName}` : null, a.cursorUrl ? `<${a.cursorUrl}|details>` : null].filter(Boolean).join(' · ')
  return [
