...
// The bot's DM channel id with the owner — where proposal/failed cards post.
// Returns null if the bot can't open the DM (missing im:write).
export async function ownerDMChannel(ownerId) {
  try {
    const d = await slack('conversations.open', BOT(), { users: ownerId }, true)
    return d.channel?.id || null
  } catch {
    return null
  }
}
...
// broadcast=true also surfaces the reply in the main channel (reply_broadcast).
export async function replyInThread(channel, threadTs, text, token, broadcast = false) {
  await slack('chat.postMessage', token, { channel, thread_ts: threadTs, text, reply_broadcast: broadcast }, true)
}
