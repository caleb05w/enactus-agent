...
// Hard owner guard — ONLY the owner may approve, skip, or retry. Defense in
// depth: the Slack route already gates on user id, and this refuses again at the
// action layer so no other code path can launch a change on someone else's click.
function denyIfNotOwner(actorId, responseUrl) {
  if (actorId === OWNER_ID) return false
  replaceMessage(responseUrl, statusBlocks(':no_entry: Only the owner can act on these.'), 'denied').catch(() => {})
  return true
}
...
export async function approveTask(actionId, repoKey, responseUrl, actorId) {
  if (denyIfNotOwner(actorId, responseUrl)) return
  await connectDB()
...
export async function skipTask(actionId, responseUrl, actorId) {
  if (denyIfNotOwner(actorId, responseUrl)) return
  await connectDB()
  const a = await AgentAction.findById(actionId).catch(() => null)
  if (!a) return
...
export async function retryTask(actionId, repoKey, responseUrl, actorId) {
  if (denyIfNotOwner(actorId, responseUrl)) return
  await connectDB()
  const a = await AgentAction.findById(actionId).catch(() => null)
  if (!a) return
...
async function notifyRequester(a) {
  ...
  if (a.source?.startsWith('ch:')) {
    await replyInThread(channelId, a.messageTs, `<@${a.requesterId}> ${friendly}`, BOT_TOKEN(), true).catch(() => {})
