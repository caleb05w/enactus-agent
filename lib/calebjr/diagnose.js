...
export async function retryTask(actionId, responseUrl) {
  await connectDB()
  const a = await AgentAction.findById(actionId).catch(() => null)
  if (!a) return
  ...
  const repo = a.repoName ? resolveRepo(a.repoName) : null
  if (!repo) {
    await replaceMessage(responseUrl, statusBlocks(`:warning: No target repo on “${a.summary}” — re-create it.`), 'no repo')
    return
  }
  const h = await handoff({ summary: a.summary, messageLink: a.messageLink, repoUrl: repo.url, repoRef: repo.ref })
  if (h.dispatched) {
...
export async function pollDispatched() {
  ...
  for (const a of dispatched) {
    ...
    if (outcome === 'done') {
      ...
      a.status = 'failed'
      await a.save()
      // Post a failure card with a Retry button (instant relaunch).
      if (LOG_CHANNEL) await postBlocks(LOG_CHANNEL, failedBlocks(a), `Task failed: ${a.summary}`).catch(() => {})
      out.failed++
    } else {
      out.running++
    }
