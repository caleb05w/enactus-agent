...
export async function processApprovals() {
  ...
  for (const a of pending) {
    ...
    if ((reactors[APPROVE] || []).includes(OWNER_ID)) {
      const h = await handoff({ summary: a.summary, messageLink: a.messageLink, repoUrl: a.repoUrl, repoRef: a.repoRef })
      if (h.dispatched) {
        a.status = 'dispatched'
      ...
      for (const it of asks) {
        ...
        if (it.redundant) {
          ...
          if (mode === 'live' && LOG_CHANNEL) {
            ...
            reason = `card posted (→ ${repo.name}); awaiting ✅`
          } else {
