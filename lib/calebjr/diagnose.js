...
import { listBotChannels, listOwnerDMs, fetchHistory, addEyes, postMessage, getReactors, getPermalink, dmOwner, replyInThread, deleteMessage, userName } from './slack'
...
async function reportReady(a, agent) {
  ...
  // Reply to whoever initiated, in-thread (channel-sourced requests only).
  if (a.source?.startsWith('ch:') && a.requesterId) {
    const channelId = a.source.slice(3)
    await replyInThread(channelId, a.messageTs, `<@${a.requesterId}> done — ${a.summary}. PR: ${prUrl}`, BOT_TOKEN()).catch(() => {})
  }
}
