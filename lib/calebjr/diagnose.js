...
export async function pollDispatched() {
  ...
  for (const a of dispatched) {
    ...
    if (outcome === 'done') {
      ...
      if (dest) {
        const ts = await postBlocks(dest, failedBlocks(a, repoOptions()), `Task failed: ${a.summary}`).catch(() => null)
        if (ts) {
      ...
      for (const it of asks) {
        ...
        if (it.redundant) {
          ...
          if (mode === 'live' && dest) {
            ...
            const cardTs = await postBlocks(dest, proposalBlocks(a, repoOptions()), `Proposed: ${it.summary}`)
            a.approvalTs = cardTs
            await a.save()
